import argparse

# 定义 train_config 参数解析
def get_train_args():
    parser = argparse.ArgumentParser(description='Training configuration')
    parser.add_argument('--data_root', type=str, default='Data', help='Root directory for data')
    parser.add_argument('--dataset_name', type=str, default='bindingdb/meta_unseen_protein/', help='Dataset name')
    parser.add_argument('--measure_name', type=str, nargs='+', default=['Y'], help='Measure names')
    parser.add_argument('--optimizer_type', type=str, default='Adam', help='Optimizer type')
    parser.add_argument('--optimizer_momentum', type=float, default=0.9, help='Optimizer momentum')
    parser.add_argument('--weight_decay', type=float, default=2.5e-5, help='Weight decay')
    parser.add_argument('--dropout', type=float, default=0.1, help='Dropout rate')
    parser.add_argument('--early_stop', action='store_false', help='Enable early stopping')
    parser.add_argument('--loss', type=str, default='FL', help='Loss function')
    parser.add_argument('--data_parallel', action='store_false', help='Enable data parallelism')
    parser.add_argument('--load', action='store_true', help='Enable loading')
    parser.add_argument('--label_index', type=int, default=2, help='Label index')
    parser.add_argument('--domain_index', type=int, default=3, help='Domain index')
    parser.add_argument('--finetune_load_path', type=str, default=None, help='Path to load fine-tuned model')
    parser.add_argument('--batch_size', type=int, default=32, help='Batch size')
    parser.add_argument('--max_epochs', type=int, default=50, help='Maximum number of epochs')
    parser.add_argument('--num_tasks', type=int, default=10000, help='Number of tasks')
    parser.add_argument('--main_metric', type=str, default='auroc', help='Main metric')
    parser.add_argument('--method', type=str, default='prototypes', help='Method')
    parser.add_argument('--adaptive', action='store_true', help='Enable adaptive learning')
    parser.add_argument('--normalize', action='store_true', help='Enable normalization')
    parser.add_argument('--num_workers', type=int, default=0, help='Number of workers for data loading')
    parser.add_argument('--lr', type=float, default=1e-4, help='Learning rate')
    parser.add_argument('--scheduler_step', type=int, default=5, help='Scheduler step')
    parser.add_argument('--scheduler_decay', type=float, default=1.0, help='Scheduler decay')
    parser.add_argument('--distance_metric', type=str, default='cosine', help='Distance metric')
    parser.add_argument('--train_ways', type=int, default=2, help='Number of ways for training')
    parser.add_argument('--train_queries', type=int, default=1, help='Number of queries for training')
    parser.add_argument('--train_shots', type=int, default=5, help='Number of shots for training')
    parser.add_argument('--test_ways', type=int, default=2, help='Number of ways for testing')
    parser.add_argument('--test_queries', type=int, default=1, help='Number of queries for testing')
    parser.add_argument('--test_shots', type=int, default=5, help='Number of shots for testing')
    parser.add_argument('--checkpoints_folder', type=str, default='checkpoints/', help='Checkpoints folder')
    parser.add_argument('--seed', type=int, default=2025, help='Random seed')
    parser.add_argument('--train_dataset_length', type=int, default=None, help='Train dataset length')
    parser.add_argument('--test_dataset_length', type=int, default=None, help='Test dataset length')
    parser.add_argument('--eval_dataset_length', type=int, default=None, help='Eval dataset length')
    parser.add_argument('--DDP', action='store_false', help='Enable Distributed Data Parallel')

    parser.add_argument('--query_file', type=str, default='Data/bindingdb/SCD1/scd1.csv', help='Query file')
    parser.add_argument('--support_file', type=str, default='Data/PubChemAnnotations FDA Drug.csv',
                        help='Support file')
    parser.add_argument('--out_file', type=str, default='out.csv',
                        help='output file')
    return parser.parse_args()

# 定义 model_config 参数解析
def get_model_args():
    parser = argparse.ArgumentParser(description='Model configuration')
    parser.add_argument('--drugban', action='store_false', help='Enable drugban')
    parser.add_argument('--muti_out', action='store_true', help='Enable multi output')
    parser.add_argument('--p_emb', type=int, default=128, help='P embedding size')
    parser.add_argument('--d_emb', type=int, default=128, help='D embedding size')
    parser.add_argument('--d_stem_channel', type=int, default=128, help='D stem channel size')
    parser.add_argument('--stem_kernel', type=int, default=1, help='Stem kernel size')
    parser.add_argument('--p_stem', action='store_true', help='Enable p_stem')
    parser.add_argument('--d_stem', action='store_true', help='Enable d_stem')
    parser.add_argument('--gate', action='store_true', help='Enable gate')
    parser.add_argument('--norm', action='store_true', help='Enable normalization in model')
    parser.add_argument('--p_stem_channel', type=int, default=128, help='P stem channel size')
    parser.add_argument('--d_channels', type=int, nargs='+', default=[128, 128, 128], help='D channels')
    parser.add_argument('--p_channels', type=int, nargs='+', default=[128, 128, 128], help='P channels')
    parser.add_argument('--d_out_channel', type=int, default=128, help='D output channel size')
    parser.add_argument('--p_out_channel', type=int, default=128, help='P output channel size')
    parser.add_argument('--out_hidden_size', type=int, default=256, help='Output hidden size')
    parser.add_argument('--layers_num', type=int, default=3, help='Number of layers')
    parser.add_argument('--binary', type=int, default=2, help='Binary flag')
    return parser.parse_args()

if __name__ == '__main__':
    train_args = get_train_args()
    model_args = get_model_args()
    print('Training Configuration:', train_args)
    print('Model Configuration:', model_args)