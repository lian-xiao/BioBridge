...
import torch
from Dti.Dti_vec import Dti_VecModel

class LightDti(pl.LightningModule):
    def __init__(self, model_config, train_config,gpu_tracker=None):
        super(LightDti, self).__init__()
        self.model = Dti_VecModel(model_config,gpu_tracker)
    ...
    def _calculate_loss(self,batch,mode='train'):
        ...
        mol,protein, labels = batch
        labels = labels.squeeze(-1)
        if self.gpu_tracker:
            self.gpu_tracker.track()
        preds = self.model(mol, protein)
