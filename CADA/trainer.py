...
class Trainer(object):
    ...
    def train(self):
        ...
        for i in range(self.epochs):
            ...
            if not self.is_da:
                ...
                if self.muda:
                    train_loss, model_loss, da_loss, epoch_lamb = self.train_mulda_epoch()
                else:
                ...
                if self.da_method == "MADA":
                    ...
                    if self.original_random:
                        ...
                        for class_idx in range(2):
                            weighted_reverse_features = softmax_output[:, class_idx].unsqueeze(1) * reverse_f
                            adv_output_src_score.append(
                                self.domain_dmm[class_idx](weighted_reverse_features).cuda()
                            )
                    else:
                        feature = torch.bmm(softmax_output.unsqueeze(2), reverse_f.unsqueeze(1))
                        feature = feature.view(-1, softmax_output.size(1) * reverse_f.size(1))
                    ...
                    if self.original_random:
                        ...
                        for class_idx in range(2):
                            weighted_reverse_features = softmax_output_t[:, class_idx].unsqueeze(1) * reverse_f_t
                            adv_output_tgt_score.append(
                                self.domain_dmm[class_idx](weighted_reverse_features).cuda()
                            )
                    ...
                    loss_cdan_tgts =[]
                    for class_idx in range(2):
                        n, loss_cdan_src = cross_entropy_logits(adv_output_src_score[class_idx],
                                             torch.zeros(self.batch_size).to(self.device))
                        n, loss_cdan_tgt = cross_entropy_logits(adv_output_tgt_score[class_idx],
                                             torch.ones(self.batch_size).to(self.device))
                        loss_cdan_srcs.append(loss_cdan_src)
                        loss_cdan_tgts.append(loss_cdan_tgt)
                    da_loss = sum(loss_cdan_srcs + loss_cdan_tgts) / 2
                else:
                    raise ValueError(f"The da method {self.da_method} is not supported")
                loss = model_loss + da_loss
            else:
    ...
    @staticmethod
    def find_ckpts(root_dir):
        for root, dirs, files in os.walk(root_dir):
            for file in files:
                if file.endswith('.ckpt') or file.endswith('.pth'):  # 假设ckpt文件以.ckpt结尾
                    return os.path.join(root, file)

    def test(self, dataloader="test",ckpt=None):
        if ckpt is not None:
            file = self.find_ckpts(ckpt)
        ...
        if dataloader == "test":
            ...
            if ckpt is not None:
                df.to_csv(os.path.join(ckpt, 'test_pred.csv'))
            else:
                df.to_csv(os.path.join(self.output_dir,'test_pred.csv'))
